...
void QountersMinus::Qounters::SpeedQounter::Update() {
    ...
    if (lastUpdated > 0.25f) {
        ...
        switch (static_cast<QountersMinus::SpeedQounterMode>(Mode)) {
            case QountersMinus::SpeedQounterMode::Average:
                rightSpeeds->Add((refs->saberManager->dyn__rightSaber()->get_bladeSpeed() + refs->saberManager->dyn__leftSaber()->get_bladeSpeed()) / 2.0f);
                averageText->set_text(StringW(FormatNumber(Average(rightSpeeds), DecimalPrecision)));
                break;
            case QountersMinus::SpeedQounterMode::Top5Sec:
                fastestSpeeds->Add((refs->saberManager->dyn__rightSaber()->get_bladeSpeed() + refs->saberManager->dyn__leftSaber()->get_bladeSpeed()) / 2.0f);
                break;
            ...
            case QountersMinus::SpeedQounterMode::SplitAverage:
                rightSpeeds->Add(refs->saberManager->dyn__rightSaber()->get_bladeSpeed());
                leftSpeeds->Add(refs->saberManager->dyn__leftSaber()->get_bladeSpeed());
                averageText->set_text(StringW(FormatNumber(Average(leftSpeeds), DecimalPrecision) + " | " + FormatNumber(Average(rightSpeeds), DecimalPrecision)));
                break;
    ...
    if (lastUpdatedFastest >= 5.0f) {
        ...
        if (fastestText) {
            ...
            fastestSpeeds->Clear();
            fastestText->set_text(StringW(FormatNumber(top, DecimalPrecision)));
        }
    }
}
