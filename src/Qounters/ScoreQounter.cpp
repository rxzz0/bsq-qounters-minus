...
void QountersMinus::Qounters::ScoreQounter::Start() {
    relativeScoreText = refs->coreGameHUDController->dyn__relativeScoreGO()->GetComponentInChildren<TMPro::TextMeshProUGUI*>();
    relativeScoreText->set_color(UnityEngine::Color::get_white());
    rankText = refs->coreGameHUDController->dyn__immediateRankGO()->GetComponent<TMPro::TextMeshProUGUI*>();
    ...
    auto scoreUIText = scoreUIController->dyn__scoreText();
    scoreUIText->get_transform()->SetParent(gameObject->get_transform(), true);
    refs->coreGameHUDController->dyn__relativeScoreGO()->get_transform()->SetParent(scoreUIText->get_transform(), true);
    refs->coreGameHUDController->dyn__immediateRankGO()->get_transform()->SetParent(scoreUIText->get_transform(), true);
    ...
    if (Mode == static_cast<int>(QountersMinus::ScoreQounterMode::RankOnly))
        UnityEngine::Object::Destroy(refs->coreGameHUDController->dyn__relativeScoreGO());
    if (Mode == static_cast<int>(QountersMinus::ScoreQounterMode::ScoreOnly))
        UnityEngine::Object::Destroy(refs->coreGameHUDController->dyn__immediateRankGO());
...
void QountersMinus::Qounters::ScoreQounter::UpdateText() {
    auto immediateRank = refs->relativeScoreAndImmediateRankCounter->get_immediateRank();
    ...
    float relativeScore = refs->relativeScoreAndImmediateRankCounter->get_relativeScore() * 100.0f;
    relativeScoreText->set_text(StringW(FormatNumber(relativeScore, DecimalPrecision) + "%"));
}
