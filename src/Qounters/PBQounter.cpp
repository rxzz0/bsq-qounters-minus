...
void QountersMinus::Qounters::PBQounter::Start() {
    ...
    maxPossibleScore = GlobalNamespace::ScoreModel::GetModifiedScoreForGameplayModifiersScoreMultiplier(
        maxRawScore,
        refs->scoreController->dyn__gameplayModifiersModel()->GetTotalMultiplier(
            refs->scoreController->dyn__gameplayModifierParams(),
            refs->scoreController->dyn__gameEnergyCounter()->get_energy()
        )
    );
    auto playerLevelStats = refs->playerData->GetPlayerLevelStatsData(refs->difficultyBeatmap);
    highScore = playerLevelStats->dyn__highScore();
    ...
    if (UnderScore) {
        ...
        auto scorePosition = scoreUIController->dyn__scoreText()->get_transform()->get_position();
...
void QountersMinus::Qounters::PBQounter::SetPersonalBest(float ratioOfMaxScore) {
    if (HideFirstScore && highScore == 0) {
        pbText->set_text(StringW("PB: --"));
    } else {
        pbText->set_text(StringW("PB: " + FormatNumber(ratioOfMaxScore * 100.0f, DecimalPrecision) + "%"));
    }
}
...
void QountersMinus::Qounters::PBQounter::OnScoreUpdated(int modifiedScore) {
    ...
    if (Mode == static_cast<int>(PBQounterMode::Relative)) {
        if (refs->relativeScoreAndImmediateRankCounter->get_relativeScore() > ((float)highScore / maxPossibleScore)) {
            if (pbText) {
