...
void QountersMinus::Qounters::Spinometer::Update() {
    ...
    leftQuaternions->Add(refs->saberManager->dyn__leftSaber()->get_transform()->get_rotation());
    rightQuaternions->Add(refs->saberManager->dyn__rightSaber()->get_transform()->get_rotation());
    ...
    basicText->set_text(StringW(nextText));
    }
}
