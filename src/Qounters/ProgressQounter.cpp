...
HMUI::ImageView* CreateRing(UnityEngine::Transform* parent) {
    auto imageGameObject = UnityEngine::GameObject::New_ctor(StringW("QountersMinusRingImage"));
    ...
    for (int i = 0; i < materials.Length(); i++) {
        if (to_utf8(csstrtostr(materials[i]->get_name())) == "UINoGlow") {
            noGlowMat = materials[i];
            break;
        }
    }
    ...
    for (int i = 0; i < sprites.Length(); i++) {
        if (to_utf8(csstrtostr(sprites[i]->get_name())) == "Circle") {
            multiplierImageSprite = sprites[i];
            break;
        }
    }
...
void QountersMinus::Qounters::ProgressQounter::Start() {
    ...
    refs->coreGameHUDController->dyn__songProgressPanelGO()->get_transform()->set_localScale(UnityEngine::Vector3(0.0f, 0.0f, 0.0f));
...
void QountersMinus::Qounters::ProgressQounter::Update() {
    ...
    auto time = audioTimeSyncController->get_songTime();
    ...
    if (
        ...
        if (Mode == static_cast<int>(QountersMinus::ProgressQounterMode::TimeInBeats)) {
            ...
            if (basicText) {
            basicText->set_text(StringW(FormatNumber(beats, 2)));
            }
        } else if (basicText) {
        ...
        progressRing->set_fillAmount((IncludeRing ? time : audioTimeSyncController->get_songTime()) / length);
        progressRing->SetVerticesDirty();
    } else if (basicText) {
        basicText->set_text(StringW(FormatNumber(100.0f * time / length, 2) + "%"));
    }
}
