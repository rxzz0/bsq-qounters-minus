...
void DeactivateChildren(std::string gameObjectName) {
    DeactivateChildren(UnityEngine::GameObject::Find(StringW(gameObjectName)));
}
...
UnityEngine::GameObject* GetGameObject(std::string name) {
    ...
    for (auto i = 0; i < allObjects; i++) {
        if (to_utf8(csstrtostr(allObjects[i]->get_name())) == name) return allObjects[i];
    }
    return nullptr;
}

UnityEngine::GameObject* GetParent(QountersMinus::QounterPosition position) {
    auto containerName = StringW("QountersMinus_Container" + std::to_string((int)position));
    ...
    if (!containerGO) {
        ...
        auto parentGO = coreGameHUDController->get_gameObject()->Find(StringW(QounterPositionData[position].parentName));
